...
Track your meeting costs at MeetingMeter! 📈💸`
  ...
  const downloadReportCard = () => {
    ...
    ctx.font = '20px Arial'
    ctx.textAlign = 'left'
    ctx.fillText(`💰 Total Cost: $${totalCost.toFixed(2)}`, 50, 300)
    ctx.fillText(`⏱️ Duration: ${formatDuration(duration)}`, 50, 340)
    ctx.fillText(`👥 Attendees: ${attendeeCount}`, 50, 380)
    ctx.fillText(`📈 Productivity Score: ${report.productivityScore}/100`, 50, 420)
    ctx.fillText(`🏆 Milestones Hit: ${milestones.length}`, 50, 460)
    ...
    // Footer
    ctx.font = '16px Arial'
    ctx.fillText('Generated by MeetingMeter', 300, 750)
